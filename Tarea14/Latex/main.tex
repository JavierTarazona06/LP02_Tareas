...
\begin{itemize}
  ...
  \item \textbf{V}*: es para valores inmediatos. Ocupa los 32 bits menos significativos de la instrucción. A no ser
...
\begin{longtable}{|p{0.1\textwidth}|p{0.2\textwidth}|p{0.1\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}|}
  ...
  \hline
  13              & \texttt{DECRE R}            & 001101                          & Decrementa un registro en 1                                        & $R \leftarrow R + 1$                       \\
  \hline
\end{longtable}
...
\begin{longtable}{|p{0.1\textwidth}|p{0.2\textwidth}|p{0.1\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}|}
  ...
  \hline
  14              & \texttt{CARGA R, M}  & \texttt{001110} & Carga desde el espacio de memoria M al registro R                                                                                                                                             & $R \leftarrow M$                                                                                                    \\
  \hline
  15              & \texttt{GUARD M, R}  & \texttt{001111} & Guarda el contenido del registro R en el espacio de memoria M                                                                                                                                 & $M \leftarrow R$                                                                                                    \\
  \hline
  *16             & \texttt{ICARGA R, V} & \texttt{010000} & Carga un valor inmediato V a un registro R. V se guarda en los 32 bits menos significativos del registro.                                                                                     & $R \leftarrow V$                                                                                                    \\
  \hline
  *17             & \texttt{ISCARGA R, V}& \texttt{010001} & Carga un valor inmediato V a un registro R.  V se guarda en los 32 bits más significativos del registro.                                                                                      & $R \leftarrow V$                                                                                                    \\
  \hline
  18              & \texttt{ISUMA R, V}  & \texttt{010010} & Suma inmediata de un valor V a un registro R                                                                                                                                                  & $R \leftarrow R + V$                                                                                                \\
  \hline
  19              & \texttt{IRESTA R, V} & \texttt{010011} & Resta inmediata de un valor V a un registro R                                                                                                                                                 & $R \leftarrow R - V$                                                                                                \\
  \hline
  20              & \texttt{IMULT R, V}  & \texttt{010100} & Multiplica un registro por un inmediato                                                                                                                                                       & $R \leftarrow R \times V$                                                                                           \\
  \hline
  21              & \texttt{IDIVI R, V}  & \texttt{010101} & Realiza la división entera de un registro R entre un valor inmediato V                                                                                                                        & $R \Leftarrow \lfloor R / V \rfloor $                                                                               \\
  \hline
  22              & \texttt{IAND R, V}   & \texttt{010110} & Conjunción AND entre un registro R y un valor inmediato V                                                                                                                                     & $ R \leftarrow R \wedge R'$                                                                                         \\
  \hline
  23              & \texttt{IOR R, V}    & \texttt{010111} & Disyunción OR entre un registro R y un valor inmediato V                                                                                                                                      & $R \leftarrow R \vee R'$                                                                                            \\
  \hline
  24              & \texttt{IXOR R, V}   & \texttt{011000} & XOR bit a bit entre un registro R y un valor inmediato V                                                                                                                                      & $R \leftarrow R \oplus R'$                                                                                          \\
  \hline
  25              & \texttt{ICOMP R, V}  & \texttt{011001} & Compara un registro R con un valor inmediato V y guarda el resultado en R3 (Registro de estado)                                                                                               & Se realiza la resta $R - V$ y se indica si el resultado es positivo ($R > V$), negativo ($R < V$) o cero ($R = V$). \\
  \hline
  26              & \texttt{APILA R}     & \texttt{011010} & Decrementa (la pila crece hacia abajo en la memoria) el registro SP (R1) y hace push del registro R en la pila (lo guarda)                                                                    & $R1 (SP) \leftarrow R1 - 1$ y guarda en la dirección que contiene SP a R:  $*SP \leftarrow R$                       \\
  \hline
  27              & \texttt{DESAPILA R}  & \texttt{011011} & Guarda el contenido de la dirección que está en SP en el registro R, y luego incrementa SP (R1). (la pila crece hacia abajo, esto es un pop)                                                  & $R \leftarrow *SP(R1)$, luego incrementa SP: $R1(SP) \leftarrow  R1 + 1 $                                           \\
  \hline
  28              & \texttt{PROCRASTINA} & \texttt{011100} & No hace nada (instrucción vacía), sirve para no hacer nada por un ciclo de reloj, se usa para temas de sincronización, o simplemente para que el PC se parezca un poco más a sus diseñadores. & Si quieres ver un anime en vez de hacer la tarea o irte a jugar r6 en vez de adelantar un trabajo.                  \\
  \hline
\end{longtable}
...
\begin{longtable}{|p{0.1\textwidth}|p{0.2\textwidth}|p{0.1\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}|}
  ...
  \textbf{Número}    & \textbf{Código}                                  & \textbf{Código Máquina Binario} & \textbf{Instrucción}                                                                                                                                                                                                          & \textbf{Descripción}                                                                                                   \\
  \hline
  29                 & \texttt{SALTA M}                                 & \texttt{011101} & Salta incondicionalmente a la dirección M, es decir, guarda la dirección M en el R0 (PC) program counter para que en el siguiente ciclo continúe desde ahí.                              & $PC(R0) \leftarrow M$                                                                                                  \\
  \hline
  30                 & \texttt{LLAMA M}                                 & \texttt{011110} & Llama a la subrutina en la dirección M, primero guarda el PC(R0) en la pila al hacer un APILA, luego pone en el PC (R0) el valor de M, donde empieza la subrutina.                        & APILA PC(R0) y luego: $PC \leftarrow M$                                                                                \\
  \hline
  31                 & \texttt{VUELVE}                                  & \texttt{011111} & Vuelve desde la subrutina desde la que se encuentre, esto al hacer DESAPILA y guardar el resultado desapilado en el PC.                                                                                                       & DESAPILA R y luego: $PC \leftarrow R$                                                                                  \\
  \hline
  32                 & \texttt{SICERO M}                                & \texttt{100000} & Salta si el resultado de la última comparación fue cero (C=1), al verificar el bit C del R3 (estado).                                                                                    & Verifica si C=1 y al confirmarlo hace un SALTA M                                                                       \\
  \hline
  33                 & \texttt{SINCERO M}                               & \texttt{100001} & Salta a M si el resultado de la última comparación no fue cero (C=0)                                                                                                                       & Verifica si C=0 en el R3 (estado) y al confirmarlo hace un SALTA M                                                     \\
  \hline
  34                 & \texttt{SIPOS M}                                 & \texttt{100010} & Salta si el resultado de la última comparación fue positivo (P=1)                                                                                                                          & Verifica si P=1 y al confirmarlo hace un SALTA M                                                                       \\
  \hline
  35                 & \texttt{SINEG M}                                 & \texttt{100011} & Salta si el resultado de la última comparación fue negativo (N=1)                                                                                                                          & Verifica si N=1 en el R3 (estado) y al confirmarlo hace un SALTA M                                                     \\
  \hline
  36                 & \texttt{SIOVERFL M}                              & \texttt{100100} & Salta si el resultado de la última comparación u operación tuvo overflow (D=1)                                                                                                            & Verifica si D=1 en R3 (estado) y al confirmarlo hace un SALTA M.                                                       \\
  \hline
  37                 & \texttt{SIREGCERO R M}                           & \texttt{100101} & Salta a M si el registro R es igual a 0.                                                                                                                                                    & Verifica si R == 0 y al confirmarlo hace un SALTA M.                                                                   \\
  \hline
  38                 & \texttt{SIREGNCERO R M}                          & \texttt{100110} & Salta a M si un registro R no es igual a cero.                                                                                                                                              & Verifica si R $\neq$ 0 y al confirmarlo hace un SALTA M.                                                                    \\
  \hline
  39                 & \texttt{SIMAYOR M}                               & \texttt{100111} & Salta si el resultado de la última operación es de mayor que, es decir, al comparar, el resultado fue positivo y no fue igual a cero. $(P = 1 \land C = 0)$                              & Verifica si los flags P y C cumplen las condiciones y al confirmarlo hace un SALTA M.                                 \\
  \hline
  40                 & \texttt{SIMENOR}                                 & \texttt{101000} & Salta si el resultado de la última operación es de menor que, es decir, al comparar, el resultado fue negativo. (N=1)                                                                    & Verifica si el flag N en R3 (estado) está activado, al confirmarlo hace un SALTA M.                                   \\
  \hline
  41                 & \texttt{PARA}                                    & \texttt{101001} & Detiene la ejecución del programa                                                                                                                                                           & Se detiene el ciclo de instrucciones, ya sea deteniendo completamente la CPU o entrando en un estado de espera (IDLE) \\
  \hline
  42                 & \texttt{INTERRUP M}                              & \texttt{101010} & Dispara una interrupción por software, guardando el estado actual del program counter en la pila junto con los flags o registros relevantes, luego salta a la rutina de interrupción M.  & Se guardan los estados del sistema, se hace un APILA PC y se SALTA M.                                                  \\
  \hline
\end{longtable}
