...
\begin{center}
\begin{tikzpicture}[node distance=1cm and 1.5cm, every node/.style={draw, minimum height=1cm}]
  \node (prog) {ProgramaBinario[]};
  \node[right=of prog] (temp) {Registro Temporal};
  \node[below=of temp] (testbit) {Comparador Bit 63};
  \node[below=of testbit] (sumador) {Sumador Relocalización};
  \node[below=of sumador] (memoria) {Memoria RAM};
  \node[left=of memoria] (pc) {Contador PC};
  \node[below=of memoria] (salida) {RAM[DirecciónBase .. +n]};
  \draw[->] (prog) -- (temp);
  \draw[->] (temp) -- (testbit);
  \draw[->] (testbit) -- (sumador);
  \draw[->] (sumador) -- (memoria);
  \draw[->] (pc) -- (memoria);
  \draw[->] (memoria) -- (salida);
\end{tikzpicture}
\end{center}

\section*{Microinstrucciones requeridas (nivel bajo)}

\begin{lstlisting}[language={[x86masm]Assembler}, basicstyle=\ttfamily\small]
CARGAR TMP, PROG[I]
TESTEARBIT TMP, 63
IF 1 THEN GOTO RELOCALIZAR
ESCRIBIR RAM[PC], TMP
INCREMENTAR PC
GOTO SIGUIENTE

RELOCALIZAR:
  EXTRAER DIRREL, TMP[0..31]
  ANADIR DIRABS, DIRREL, DirecciónBase
  REEMPLAZAR TMP[0..31] ← DIRABS
  LIMPIAR BIT TMP, 63
  ESCRIBIR RAM[PC], TMP
  INCREMENTAR PC
